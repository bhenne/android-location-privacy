...
/**
 * The service class that manages LocationProviders and issues location updates
 * and alerts.
 * 
 * {@hide}
 */
public class LocationManagerService extends ILocationManager.Stub implements
	...
	private static final String ACCESS_MOCK_LOCATION = android.Manifest.permission.ACCESS_MOCK_LOCATION;
	private static final String ACCESS_LOCATION_EXTRA_COMMANDS = android.Manifest.permission.ACCESS_LOCATION_EXTRA_COMMANDS;
	...
	// Cache the real providers for use in addTestProvider() and
	// removeTestProvider()
	 ...
	 */
	private final ArrayList<LocationProviderInterface> mProviders = new ArrayList<LocationProviderInterface>();
	private final HashMap<String, LocationProviderInterface> mProvidersByName = new HashMap<String, LocationProviderInterface>();
	...
	private HashMap<PendingIntent, ProximityAlert> mProximityAlerts = new HashMap<PendingIntent, ProximityAlert>();
	private HashSet<ProximityAlert> mProximitiesEntered = new HashSet<ProximityAlert>();
	...
	/**
	 * A wrapper class holding either an ILocationListener or a PendingIntent to
	...
	private final class Receiver implements IBinder.DeathRecipient,
		...
		public boolean callStatusChangedLocked(String provider, int status,
				Bundle extras) {
			if (mListener != null) {
				try {
					synchronized (this) {
						...
						if (mListener != mProximityListener) {
							// call this after broadcasting so we do not
							// increment
				...
				statusChanged.putExtra(LocationManager.KEY_STATUS_CHANGED,
						status);
				try {
					synchronized (this) {
						// synchronize to ensure
						// incrementPendingBroadcastsLocked()
						// is called before decrementPendingBroadcasts()
						mPendingIntent.send(mContext, 0, statusChanged, this,
								mLocationHandler, requiredPermissions);
			...
			if (mListener != null) {
				...
				try {
					synchronized (this) {
						...
						// is called before decrementPendingBroadcasts()
						if(calcLoc != null){
							mPendingIntent.send(mContext, 0, locationChanged, this,
									mLocationHandler, requiredPermissions);
							// call this after broadcasting so we do not increment
							// if we throw an exeption.
							incrementPendingBroadcastsLocked();
		...
		public boolean callProviderEnabledLocked(String provider,
				boolean enabled) {
			if (mListener != null) {
				try {
					synchronized (this) {
				...
				providerIntent.putExtra(LocationManager.KEY_PROVIDER_ENABLED,
						enabled);
				try {
					synchronized (this) {
						// synchronize to ensure
						// incrementPendingBroadcastsLocked()
						// is called before decrementPendingBroadcasts()
						mPendingIntent.send(mContext, 0, providerIntent, this,
								mLocationHandler, requiredPermissions);
						// call this after broadcasting so we do not increment
						// if we throw an exeption.
						incrementPendingBroadcastsLocked();
		...
		// this must be called while synchronized by caller in a synchronized
		// block
	...
	public void locationCallbackFinished(ILocationListener listener) {
		...
		// LocationListener was removed when it had a pending broadcast and
		// should
		// not be added back.
	...
	public void setGPSSource(String device) {
		synchronized (mLock) {
			if (mGpsLocationProvider != null
					&& mProvidersByName.containsKey(mGpsLocationProvider
							.getName())) {
				Slog.i(TAG, "Disable and removing provider "
						+ mGpsLocationProvider.getName());
			...
			if ("0".equals(device)) {
				...
				BTGpsLocationProvider gpsProvider = new BTGpsLocationProvider(
						mContext, this);
				mGpsStatusProvider = gpsProvider.getGpsStatusProvider();
	...
	private void _loadProvidersLocked() {
		...
		String btDevice = Settings.System.getString(
				mContext.getContentResolver(),
		...
		if (mNetworkLocationProviderPackageName != null
				&& pm.resolveService(new Intent(
						mNetworkLocationProviderPackageName), 0) != null) {
	...
	/**
	 * @param context
	 *            the context that the LocationManagerService runs in
	 */
	public LocationManagerService(Context context) {
		super();
		mContext = context;
		Resources resources = context.getResources();
		mNetworkLocationProviderPackageName = resources
				.getString(com.android.internal.R.string.config_networkLocationProvider);
		mGeocodeProviderPackageName = resources
				.getString(com.android.internal.R.string.config_geocodeProvider);
	...
	private void initialize() {
		// Create a wake lock, needs to be done before calling loadProviders()
		// below
	...
	private String checkPermissionsSafe(String provider, String lastPermission) {
		...
		if (mContext.checkCallingOrSelfPermission(ACCESS_COARSE_LOCATION) == PackageManager.PERMISSION_GRANTED) {
			return ACCESS_FINE_LOCATION.equals(lastPermission) ? lastPermission
	...
	private boolean isAllowedProviderSafe(String provider) {
		if ((LocationManager.GPS_PROVIDER.equals(provider) || LocationManager.PASSIVE_PROVIDER
				.equals(provider))
	...
	private List<String> _getProvidersLocked(Criteria criteria,
			boolean enabledOnly) {
		if (LOCAL_LOGV) {
	...
	private class LpPowerComparator implements
			Comparator<LocationProviderInterface> {
		public int compare(LocationProviderInterface l1,
				LocationProviderInterface l2) {
			// Smaller is better
			return (l1.getPowerRequirement() - l2.getPowerRequirement());
		}

		public boolean equals(LocationProviderInterface l1,
				LocationProviderInterface l2) {
			return (l1.getPowerRequirement() == l2.getPowerRequirement());
		}
	}

	private class LpAccuracyComparator implements
			Comparator<LocationProviderInterface> {
		public int compare(LocationProviderInterface l1,
				LocationProviderInterface l2) {
			// Smaller is better
	...
	private class LpCapabilityComparator implements
			Comparator<LocationProviderInterface> {
	...
	private LocationProviderInterface best(List<String> providerNames) {
		...
		synchronized (mLock) {
			providers = new ArrayList<LocationProviderInterface>(
					providerNames.size());
	...
	private void updateProviderListenersLocked(String provider, boolean enabled) {
		...
		if (records != null) {
			...
			for (int i = 0; i < N; i++) {
				...
				if (!record.mReceiver.callProviderEnabledLocked(provider,
						enabled)) {
	...
	private class UpdateRecord {
		...
		UpdateRecord(String provider, long minTime, float minDistance,
				boolean singleShot, Receiver receiver, int uid) {
			mProvider = provider;
			mReceiver = receiver;
			mMinTime = minTime;
		...
		/**
		 * Method to be called when a record will no longer be used. Calling
		 * this multiple times must have the same effect as calling it once.
		 */
		void disposeLocked() {
			ArrayList<UpdateRecord> records = mRecordsByProvider
					.get(this.mProvider);
			if (records != null) {
				records.remove(this);
			}
		...
		void dump(PrintWriter pw, String prefix) {
			...
			pw.println(prefix + "mProvider=" + mProvider + " mReceiver="
					+ mReceiver);
			pw.println(prefix + "mMinTime=" + mMinTime + " mMinDistance="
					+ mMinDistance);
	...
	private boolean providerHasListener(String provider, int uid,
			Receiver excludedReceiver) {
		ArrayList<UpdateRecord> records = mRecordsByProvider.get(provider);
		if (records != null) {
			for (int i = records.size() - 1; i >= 0; i--) {
				UpdateRecord record = records.get(i);
	...
	public void requestLocationUpdates(String provider, Criteria criteria,
		...
		if (criteria != null) {
			// FIXME - should we consider using multiple providers
			// simultaneously
			...
			if (provider == null) {
				throw new IllegalArgumentException(
						"no providers found for criteria");
			}
		}
		try {
			synchronized (mLock) {
	...
	void validatePendingIntent(PendingIntent intent) {
		...
		// throw new
		// SecurityException("Given Intent does not require a specific package: "
		// + intent);
	}
	...
	private void requestLocationUpdatesLocked(String provider, long minTime,
			float minDistance, boolean singleShot, Receiver receiver) {
		...
		try {
			UpdateRecord r = new UpdateRecord(provider, minTime, minDistance,
					singleShot, receiver, callingUid);
			...
			if (isProviderEnabled) {
				...
				// try requesting single shot if singleShot is true, and fall
				// back to
				// regular location tracking if requestSingleShotFix() is not
				// supported
			...
			if (LOCAL_LOGV) {
				Slog.v(TAG, "_requestLocationUpdates: provider = " + provider
						+ " listener = " + receiver);
			}
		} finally {
			Binder.restoreCallingIdentity(identity);
		...
		try {
			...
			if (oldRecords != null) {
				...
				for (UpdateRecord record : oldRecords.values()) {
					if (!providerHasListener(record.mProvider, callingUid,
							receiver)) {
						LocationProviderInterface p = mProvidersByName
								.get(record.mProvider);
						if (p != null) {
			...
			// other listeners; if one does, inform it of the new smallest
			// minTime
			...
			for (String provider : providers) {
				...
				ArrayList<UpdateRecord> recordsForProvider = mRecordsByProvider
						.get(provider);
	...
	public void removeGpsStatusListener(IGpsStatusListener listener) {
		synchronized (mLock) {
			try {
				...
				Slog.e(TAG,
						"mGpsStatusProvider.removeGpsStatusListener failed", e);
			}
		}
	}
	...
	public boolean sendExtraCommand(String provider, String command,
			Bundle extras) {
		if (provider == null) {
			// throw NullPointerException to remain compatible with previous
			// implementation
		...
		if ((mContext
				.checkCallingOrSelfPermission(ACCESS_LOCATION_EXTRA_COMMANDS) != PackageManager.PERMISSION_GRANTED)) {
			throw new SecurityException(
	...
	public boolean sendNiResponse(int notifId, int userResponse) {
		...
		try {
			...
			Slog.e(TAG,
					"RemoteException in LocationManagerService.sendNiResponse");
			return false;
		}
	}
	...
	class ProximityAlert {
		...
		void dump(PrintWriter pw, String prefix) {
			pw.println(prefix + this);
			pw.println(prefix + "mLatitude=" + mLatitude + " mLongitude="
					+ mLongitude);
			pw.println(prefix + "mRadius=" + mRadius + " mExpiration="
					+ mExpiration);
	...
	class ProximityListener extends ILocationListener.Stub implements
			PendingIntent.OnFinished {
		...
		public void onLocationChanged(Location loc) {
			...
			// If Gps is available, then ignore updates from
			// NetworkLocationProvider
			if (loc.getProvider().equals(LocationManager.GPS_PROVIDER)) {
				isGpsAvailable = true;
			}
			if (isGpsAvailable
					&& loc.getProvider().equals(
							LocationManager.NETWORK_PROVIDER)) {
				return;
			}
			...
			for (ProximityAlert alert : mProximityAlerts.values()) {
				...
				if ((expiration == -1) || (now <= expiration)) {
					...
					boolean inProximity = alert.isInProximity(latitude,
							longitude, accuracy);
					if (!entered && inProximity) {
						...
						try {
							synchronized (this) {
								...
								// is called before decrementPendingBroadcasts()
								intent.send(mContext, 0, enteredIntent, this,
										mLocationHandler, ACCESS_FINE_LOCATION);
								// call this after broadcasting so we do not
								// increment
								// if we throw an exeption.
								incrementPendingBroadcasts();
							...
							if (LOCAL_LOGV) {
								Slog.v(TAG, "Canceled proximity alert: "
										+ alert, e);
							}
						...
						try {
							synchronized (this) {
								...
								// incrementPendingBroadcasts()
								// is called before decrementPendingBroadcasts()
								intent.send(mContext, 0, exitedIntent, this,
										mLocationHandler, ACCESS_FINE_LOCATION);
								// call this after broadcasting so we do not
								// increment
								// if we throw an exeption.
	...
	public void addProximityAlert(double latitude, double longitude,
		...
		try {
			synchronized (mLock) {
				addProximityAlertLocked(latitude, longitude, radius,
						expiration, intent, uid);
			}
		} catch (SecurityException se) {
			throw se;
		} catch (IllegalArgumentException iae) {
	...
	private void addProximityAlertLocked(double latitude, double longitude,
		...
		if (LOCAL_LOGV) {
			Slog.v(TAG, "addProximityAlert: latitude = " + latitude
					+ ", longitude = " + longitude + ", expiration = "
					+ expiration + ", intent = " + intent);
		}
	 ...
	 *             if the provider is not allowed to be accessed by the caller
	 */
	...
	public void reportLocation(Location location, boolean passive) {
		...
		Message m = Message.obtain(mLocationHandler, MESSAGE_LOCATION_CHANGED,
				location);
		m.arg1 = (passive ? 1 : 0);
	...
	private static boolean shouldBroadcastSafe(Location loc, Location lastLoc,
			UpdateRecord record) {
		...
		// TODO - we should probably still rebroadcast if user explicitly sets a
		// minTime > 0
	...
	private void handleLocationChangedLocked(Location location, boolean passive) {
		String provider = (passive ? LocationManager.PASSIVE_PROVIDER
		...
		for (int i = 0; i < N; i++) {
			...
			if ((newStatusUpdateTime > prevStatusUpdateTime)
				...
				if (!receiver.callStatusChangedLocked(provider, status, extras)) {
					receiverDead = true;
					Slog.w(TAG, "RemoteException calling onStatusChanged on "
							+ receiver);
				}
			}

			// remove receiver if it is dead or we just processed a single shot
			// request
		...
		public void handleMessage(Message msg) {
			try {
				if (msg.what == MESSAGE_LOCATION_CHANGED) {
					...
					// reconnect to external providers after their packages have
					// been updated
					if (mNetworkLocationProvider != null
							&& mNetworkLocationProviderPackageName
									.startsWith(packageDot)) {
				...
				// Log, don't crash!
				Slog.e(TAG,
						"Exception in LocationWorkerHandler.handleMessage:", e);
			}
		}
	}
	...
	private final BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			boolean queryRestart = action
					.equals(Intent.ACTION_QUERY_PACKAGE_RESTART);
			if (queryRestart
				...
				synchronized (mLock) {
					...
					if (action
						...
						uidList = new int[] { intent.getIntExtra(
								Intent.EXTRA_UID, -1) };
					}
					...
					for (int uid : uidList) {
						if (uid >= 0) {
							...
							for (ArrayList<UpdateRecord> i : mRecordsByProvider
									.values()) {
								for (int j = i.size() - 1; j >= 0; j--) {
									UpdateRecord ur = i.get(j);
									if (ur.mReceiver.isPendingIntent()
											&& ur.mUid == uid) {
										if (queryRestart) {
							...
							if (removedAlerts != null) {
								for (int i = removedAlerts.size() - 1; i >= 0; i--) {
									removeProximityAlertLocked(removedAlerts
											.get(i).mIntent);
								}
							}
						}
				...
				NetworkInfo info = (NetworkInfo) intent
						.getExtra(ConnectivityManager.EXTRA_NETWORK_INFO);
	...
	private final PackageMonitor mPackageMonitor = new PackageMonitor() {
		...
		public void onPackageUpdateFinished(String packageName, int uid) {
			...
			Message.obtain(mLocationHandler, MESSAGE_PACKAGE_UPDATED,
		...
		synchronized (mWakeLock) {
			if (mPendingBroadcasts++ == 0) {
				try {
					...
					// This is to catch a runtime exception thrown when we try
					// to release an
	...
	public String getFromLocation(double latitude, double longitude,
		...
		if (mGeocodeProvider != null) {
			return mGeocodeProvider.getFromLocation(latitude, longitude,
	...
	public String getFromLocationName(String locationName,
		...
		if (mGeocodeProvider != null) {
			return mGeocodeProvider.getFromLocationName(locationName,
					lowerLeftLatitude, lowerLeftLongitude, upperRightLatitude,
					upperRightLongitude, maxResults, params, addrs);
		}
		return null;
	}
	...
	private void checkMockPermissionsSafe() {
		boolean allowMocks = Settings.Secure.getInt(
				mContext.getContentResolver(),
	...
	public void addTestProvider(String name, boolean requiresNetwork,
		...
		if (LocationManager.PASSIVE_PROVIDER.equals(name)) {
			throw new IllegalArgumentException(
					"Cannot mock the passive location provider");
		}

		long identity = Binder.clearCallingIdentity();
		synchronized (mLock) {
			MockProvider provider = new MockProvider(name, this,
					requiresNetwork, requiresSatellite, requiresCell,
					hasMonetaryCost, supportsAltitude, supportsSpeed,
					supportsBearing, powerRequirement, accuracy);
			// remove the real provider if we are replacing GPS or network
			// provider
	...
	public void removeTestProvider(String provider) {
		...
		synchronized (mLock) {
			...
			// reinstall real provider if we were mocking GPS or network
			// provider
			if (LocationManager.GPS_PROVIDER.equals(provider)
					&& mGpsLocationProvider != null) {
	...
	public void setTestProviderLocation(String provider, Location loc) {
		...
		synchronized (mLock) {
			...
			// clear calling identity so INSTALL_LOCATION_PROVIDER permission is
			// not required
			long identity = Binder.clearCallingIdentity();
	...
	protected void dump(FileDescriptor fd, PrintWriter pw, String[] args) {
		...
		synchronized (mLock) {
			...
			for (Receiver i : mReceivers.values()) {
				pw.println("    " + i + ":");
				for (Map.Entry<String, UpdateRecord> j : i.mUpdateRecords
						.entrySet()) {
					pw.println("      " + j.getKey() + ":");
					j.getValue().dump(pw, "        ");
				}
			...
			for (Map.Entry<String, ArrayList<UpdateRecord>> i : mRecordsByProvider
					.entrySet()) {
				pw.println("    " + i.getKey() + ":");
			...
			if (mProximityAlerts.size() > 0) {
				...
				for (Map.Entry<PendingIntent, ProximityAlert> i : mProximityAlerts
						.entrySet()) {
					pw.println("    " + i.getKey() + ":");
					i.getValue().dump(pw, "      ");
				}
			...
			if (mMockProviders.size() > 0) {
				...
				for (Map.Entry<String, MockProvider> i : mMockProviders
						.entrySet()) {
					i.getValue().dump(pw, "      ");
				}
			}
